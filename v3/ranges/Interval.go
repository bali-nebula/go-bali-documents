...
func (v *interval_[V]) GetIterator() fra.IteratorLike[V] {
...
func (v *interval_[V]) validateInterval() {
	...
	if v.minimum_.IsDefined() && v.maximum_.IsDefined() {
		var collator = fra.CollatorClass[V]().Collator()
		if collator.RankValues(v.minimum_, v.maximum_) != fra.LesserRank {
			var message = fmt.Sprintf(
...
func (v *iterator_[V]) GetClass() fra.IteratorClassLike[V] {
